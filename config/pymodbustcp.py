...
try:
    if len(sys.argv) < 4:
        print(
            json.dumps(
                {
                    "success": False,
                    "message": "Usage: pymodbustcp.py <meter_id> <ip> <port> <quantity> <Slave ID>",
                }
            )
        )
        sys.exit(1)
    ...
    def read_float_register(client, start_address):
        result = client.read_holding_registers(address=start_address, count=quan, unit=sid)
        if result.isError():
            return None
    ...
    if not client.connect():
        print(
            json.dumps(
                {"success": False, "message": f"Connection failed to {ip}:{port}"}
            )
        )
        exit(1)
    ...
    payload = {"meter_id": meter_id, "datetime": timestamp, "data": data}
    ...
    print(
        json.dumps({"success": False, "message": "Not found meter.", "output": str(e)})
    )
    exit(1)
